...
export default function MenuPage() {
  ...
  const ProductoCard = ({ producto }) => (
    <div
      style={{ background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0', transition: 'all 0.2s', cursor: 'pointer', position: 'relative' }}
      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(46,125,50,0.12)' }}
      onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 2px 12px rgba(0,0,0,0.06)' }}
    >
      <button onClick={() => toggleFavorito(producto)} style={{ position: 'absolute', top: '12px', right: '12px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.1rem' }}>
        {esFavorito(producto.id) ? '⭐' : '☆'}
      </button>
      <div style={{ fontSize: '2.5rem', marginBottom: '12px', textAlign: 'center' }}>{producto.emoji}</div>
      <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#1e293b', marginBottom: '6px', lineHeight: 1.3, paddingRight: '20px' }}>{producto.nombre}</div>
      <div style={{ color: '#2e7d32', fontWeight: 800, fontSize: '1.1rem', marginBottom: '12px' }}>{producto.precio.toFixed(2)}€</div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ background: 'white', padding: '16px 28px', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: '400px' }}>
            ...
            <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Busca bocadillos, cafés..."
          ...
          {vista === 'menu' && (
            <>
              <div style={{ background: 'linear-gradient(135deg, #1b5e20, #2e7d32)', borderRadius: '20px', padding: '28px 32px', marginBottom: '24px', position: 'relative', overflow: 'hidden' }}>
                ...
                <p style={{ color: '#8bc34a', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', margin: '0 0 6px' }}>{saludo}, {usuario?.nombre || 'Usuario'} ✨</p>
                <h2 style={{ color: 'white', fontSize: '1.8rem', fontWeight: 800, margin: '0 0 16px' }}>¿Qué te apetece hoy?</h2>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {turnos.map(t => (
                    <button key={t.label} onClick={() => setTurnoSeleccionado(t.label)}
                ...
                pedidosHistorial.map(pedido => (
                  <div key={pedido.id} style={{ background: 'white', borderRadius: '16px', padding: '20px', marginBottom: '12px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', borderLeft: '4px solid #2e7d32' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      ...
                      <span style={{ background: '#d4edda', color: '#2e7d32', padding: '4px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>{pedido.estado.replace('_', ' ')}</span>
                    </div>
                    <div style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '8px' }}>{pedido.items.join(', ')}</div>
